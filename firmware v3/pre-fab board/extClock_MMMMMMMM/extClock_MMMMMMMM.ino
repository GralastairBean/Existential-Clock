...
void loop() {
  ...
  if (now >= deathTime) {
    // Show "0" minutes when expired
    displayNumber(0);
  } else {
    TimeSpan remaining = deathTime - now;
    unsigned long long totalSecs = remaining.totalseconds();
    unsigned long long totalMins = totalSecs / 60ULL;  // convert to minutes
    displayNumber(totalMins);
  }

  delay(1000); // update once per second is fine, still based on RTC
}
